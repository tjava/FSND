...
def venues():
  ...
  all_datas =  Venue.query.with_entities(Venue.city, Venue.state).group_by(Venue.city, Venue.state).all()
  new_data = []

  for all_data in all_datas:
    venues = Venue.query.filter_by(state=all_data.state).filter_by(city=all_data.city).all()
    all_data_venue = []
    for venue in venues:
      all_data_venue.append({
        "id": venue.id,
        "name": venue.name,
...
def search_venues():
  ...
  results = Venue.query.filter(Venue.name.ilike(f'%{search}%')).all()
  new_data = []
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error: 
    flash('An error occurred. Venue {venue_id} delete failed.')
  if not error: 
...
def edit_artist(artist_id):
  ...
  new_venue = Artist.query.get(artist_id)

  if not new_venue: 
    return render_template('errors/404.html')

  if new_venue: 
    form.name.data = new_venue.name
    form.genres.data = new_venue.genres
    form.city.data = new_venue.city
    form.state.data = new_venue.state
    form.phone.data = new_venue.phone
    form.website_link.data = new_venue.website_link
    form.facebook_link.data = new_venue.facebook_link
    form.seeking_venue.data = new_venue.seeking_venue
    form.seeking_description.data = new_venue.seeking_description
    form.image_link.data = new_venue.image_link

  return render_template('forms/edit_artist.html', form=form, artist=new_venue)
...
def edit_artist_submission(artist_id):
  ...
  try: 
    ...
    db.session.close()
  if error: 
    flash('An error occurred. Artist update failed.')
  if not error: 
...
def edit_venue(venue_id):
  ...
  if new_venue: 
    form.name.data = new_venue.name
    form.genres.data = new_venue.genres
    form.address.data = new_venue.address
    form.city.data = new_venue.city
    form.state.data = new_venue.state
    form.phone.data = new_venue.phone
    form.website_link.data = new_venue.website_link
    form.facebook_link.data = new_venue.facebook_link
    form.seeking_talent.data = new_venue.seeking_talent
    form.seeking_description.data = new_venue.seeking_description
    form.image_link.data = new_venue.image_link
...
def edit_venue_submission(venue_id):
  ...
  try: 
    ...
    db.session.close()
  if error: 
    flash('An error occurred. Venue update failed.')
  if not error: 
...
def shows():
  ...
  # TODO: replace with real venues data.
  query_shows = Show.query.join(Artist).join(Venue).all()
  new_data = []
